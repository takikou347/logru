...
import type { KakeiboSettlementSummary, KakeiboTransfer } from "./api";
...
export function SettlementPanel({ groups, group, me }: { groups: GroupSummary[]; group: GroupSummary; me: Me }) {
  ...
  return (
    <Panel title="精算">
      <LoadableSection query={settlement} what="精算" skeleton={<PanelSkeleton lines={3} />}>
        {(data: KakeiboSettlementSummary) => {
          const { balances, transfers } = data;
          const settlements = data.settlements.filter((s) => !pending.has(s.id));
          return (
            <>
                    ...
                    <PanelRow key={s.id}>
                      <span className="flex min-w-0 flex-col gap-0.5">
                        <span className="min-w-0 truncate">
                          {kakeiboPersonName(s.fromUser, group.members, me)} →{" "}
                          {kakeiboPersonName(s.toUser, group.members, me)}
                        </span>
