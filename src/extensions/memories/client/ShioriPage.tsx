...
function Shiori({ detail, me, group }: ShellProps) {
  ...
  const now = Date.now();
  const left = Math.max(0, Math.round((Date.parse(dayKeyIn(memory.startsAt, memory.timeZone)) - Date.parse(dayKeyIn(now, memory.timeZone))) / 86_400_000));
  const todoLeft = items.filter((i) => i.kind === "todo" && !i.doneAt).length;
  const count = (k: ItemKind) => items.filter((i) => i.kind === k).length;

  return (
    <>
      <section className="glass relative grid grid-cols-[1fr_auto] items-end gap-x-3 gap-y-2 rounded-panel px-5 pt-5 pb-4" aria-label="期間">
        <span aria-hidden="true" className="absolute -top-px left-6 h-[30px] w-[18px] bg-primary [clip-path:polygon(0_0,100%_0,100%_100%,50%_76%,0_100%)]" />
      ...
      <div className="grid grid-cols-3 gap-2" role="tablist" aria-label="しおりの中身">
        {KINDS.map((k) => (
          <Chip key={k.value} role="tab" aria-selected={kind === k.value} aria-pressed={kind === k.value} className="justify-center px-2" onClick={() => setKind(k.value)}>
      ...
      {days.map((day, i) => {
        ...
        return (
          <Panel key={day} aria-label={`${day} のしおり`}>
            ...
            <ul>
              {dayEvents.map((e) => (
                <li key={e.id} className="grid min-h-11 grid-cols-[46px_1fr] items-center border-t border-line text-sm text-ink-2">
                  <time>{e.allDay ? "終日" : formatClock(e.startsAt, memory.timeZone)}</time>
                  <span className="flex items-center gap-2 before:h-[18px] before:w-[3px] before:rounded-sm before:bg-(--c) before:content-['']" style={{ ["--c" as string]: `var(--${group?.color ?? "nezumi"})` }}>
                    {e.title}
                  </span>
                </li>
              ))}
...
function Todos({ detail, group, me }: ListProps) {
  ...
  const sorted = [...todos].sort((a, b) => Number(Boolean(a.doneAt)) - Number(Boolean(b.doneAt)) || (a.dueOn ?? "9").localeCompare(b.dueOn ?? "9"));
  return (
...
function ItemRows({ items, detail, group, me }: { items: MemoryItem[]; detail: MemoryDetail; group: GroupSummary | undefined; me: Me }) {
  ...
  return (
    <ul>
      {items.map((item) => {
        const who = item.kind === "wish" ? group?.members.find((m) => m.id === item.createdBy) : group?.members.find((m) => m.id === item.assigneeId);
        ...
        return (
          <li key={item.id} className="grid min-h-[52px] grid-cols-[34px_1fr_auto] items-center gap-1 border-t border-line text-sm">
            ...
            <span className="flex items-center gap-1">
              {who ? (
                <InitialAvatar person={{ id: who.id, name: who.name, color: memberColor(who.id, who.userColor, me.colorPrefs) }} />
              ) : (
...
function AddRow({
  ...
  async function submit(e: FormEvent) {
    ...
    await add.mutateAsync({ kind, title: title.trim(), dayIndex, place: place.trim() || null, assigneeId: assigneeId || null, dueOn: dueOn || null });
    setTitle("");
    setPlace("");
    setDueOn("");
  }
  ...
  if (!open) {
    return (
      <button type="button" className="flex min-h-11 w-full items-center gap-2.5 border-t border-line text-sm text-ink-2" onClick={() => setOpen(true)}>
  ...
  return (
    <form className="flex flex-col gap-2 border-t border-line pt-2.5" onSubmit={submit}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {withDue && <Input type="date" value={dueOn} aria-label="期限" className="w-auto flex-1" onChange={(e) => setDueOn(e.target.value)} />}
      </div>
