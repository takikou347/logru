 ...
 */
export function Slideshow({ records, timeZone, onClose }: { records: MemoryRecord[]; timeZone: string; onClose: () => void }) {
  ...
  return (
    <ResponsiveSheet title="ひとコマを再生" onClose={onClose}>
      ...
      <div className="flex items-center justify-center gap-2">
        <Button variant="ghost" size="icon" aria-label="前のひとコマ" onClick={() => setIndex((i) => (i - 1 + records.length) % records.length)}>
