...
test("招待した相手には返事待ちの枠線で出て、参加すると塗り、参加しないと薄く取り消し線になる", async ({ page, browser }) => {
  ...
  await expect(mika!.getByRole("region", { name: "月の表" }).locator('[data-response="pending"]').first()).toBeAttached();
  ...
  await page.reload();
  sheet = await openItem(page, "映画");
  await expect(sheet.getByRole("region", { name: "参加者" }).getByRole("listitem").filter({ hasText: "みか" })).toContainText("参加しない");
});

test("招待された人は予定を直せ、直した内容は作った人にも出る。招待されていないメンバーは見るだけ", async ({ page, browser }) => {
...
test("「表示する人」で相手だけを出すと、相手が参加するか返事待ちの予定が出て、参加しないと返した予定は出ない", async ({ page, browser }) => {
  test.setTimeout(120_000);
  const [mika] = await shareGroup(page, browser, ["みか"]);
  ...
  await people.getByRole("group", { name: "ふたり のメンバー" }).getByRole("button", { name: "自分", exact: true }).click();
  await page.keyboard.press("Escape");
...
test("API でも、招待できるのはグループのメンバーだけ。直せるのは作った人と招待された人、消せるのは作った人だけ", async ({ request }) => {
  ...
  const base = { groupId: group.id, title: "映画", allDay: false, startsAt: Date.now(), endsAt: Date.now() + 3_600_000, memo: null };

  // グループの外の人は招待できない
  const outside = await request.post("/api/events", { headers: owner.headers, data: { ...base, attendeeIds: [outsider.id] } });
  expect(outside.status()).toBe(400);

  const created = await request.post("/api/events", { headers: owner.headers, data: { ...base, attendeeIds: [invitee.id] } });
  ...
  const patchOutside = await request.patch(`/api/events/${event.id}`, { headers: invitee.headers, data: { attendeeIds: [outsider.id] } });
  ...
  expect((await request.patch(`/api/events/${event.id}`, { headers: member.headers, data: { title: "x" } })).status()).toBe(403);
  expect((await request.put(`/api/events/${event.id}/response`, { headers: member.headers, data: { response: "accepted" } })).status()).toBe(403);
  ...
  const edited = await request.patch(`/api/events/${event.id}`, { headers: invitee.headers, data: { title: "映画と夕飯", attendeeIds: [member.id] } });
  ...
  // 作った人は、自分に返事をしない
  expect((await request.put(`/api/events/${event.id}/response`, { headers: owner.headers, data: { response: "declined" } })).status()).toBe(403);

  // 招待されたメンバーが返事をし、グループを抜けると参加者から外れる
  const answered = await request.put(`/api/events/${event.id}/response`, { headers: member.headers, data: { response: "accepted" } });
