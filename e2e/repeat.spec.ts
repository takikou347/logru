...
test("朝 7 時の予定で毎月を選ぶと、来月も同じ日に出る。UTC では前日になる時刻。0068", async ({ page }) => {
  // 2026-11-01 06:00 は日本時間で 11 月 1 日。新しい予定の既定は次の正時なので 7:00 になる。UTC では前日 10/31 21:00
  await page.clock.setFixedTime(new Date("2026-11-01T06:00:00+09:00"));
  await page.reload();
  await expect(page.getByRole("region", { name: "月の表" })).toBeVisible();
  ...
  await sheet.getByRole("radio", { name: "毎月" }).click();
  await sheet.getByRole("button", { name: "保存する" }).click();
  await expect(page.getByText("予定を足しました")).toBeVisible();

  // 来月へ移り、同じ 1 日に出る。UTC の暦のままなら 11 月が飛ばされ、日にちもずれていた。0068
  await page.goto("/?view=day&date=2026-12-01");
  const day = page.getByRole("region", { name: "日の予定" });
  await expect(day.getByRole("button", { name: /家賃の支払い/ })).toBeVisible();
  await page.goto("/?view=day&date=2026-12-02");
  await expect(day.getByRole("button", { name: /家賃の支払い/ })).toHaveCount(0);
});
