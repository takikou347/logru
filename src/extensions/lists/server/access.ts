...
/**
 * そのグループのメンバーで、リストの拡張が有効かを確かめる。違えば 404。
 * 無効にしたグループのデータは消さないが、見せない。
 * @param db D1 を包んだ Drizzle
 * @param userId 利用者の ID
 * @param groupId グループの ID
 */
export async function requireListsGroup(db: DB, userId: string, groupId: string): Promise<void> {
  const ok = await usableGroupIds(db, userId, [groupId]);
  if (ok.length === 0) throw new HttpError(404, "見つかりません。");
}

/**
 * 利用者がリストに使えるグループの ID。0019
 *
 * 使うかどうかは、自分だけのグループの切り替えで持つ。使わないなら、どのグループも使えない。
 * 使うなら、自分だけのグループと、リストを有効にした共有のグループ。自分だけのグループは「共有しない」に当たる。
 *
 * @param db D1 を包んだ Drizzle
 * @param userId 利用者の ID
  ...
  const rows = await db
    ...
    .innerJoin(
      ...
      and(
        ...
        eq(groupExtensions.extensionKey, listsManifest.key),
