...
test("予定を足すと、選んだ日の欄に出る", async ({ page }) => {
  ...
  const panel = page.locator(".daypanel");
...
test("予定を直せる", async ({ page }) => {
  ...
  await page.locator(".daypanel").getByRole("button", { name: /買い物/ }).click();
  ...
  await expect(page.locator(".daypanel").getByRole("button", { name: /夕飯の買い出し/ })).toBeVisible();
});
...
test("終日の予定は、終わりの日まで出る", async ({ page }) => {
  ...
  await sheet.getByRole("button", { name: "保存する" }).click();
  await expect(page.locator(".daypanel").getByRole("button", { name: /終日\s*旅行/ })).toBeVisible();
  await page.goto(`/?date=${endKey}`);
  await expect(page.locator(".daypanel").getByRole("button", { name: /旅行/ })).toBeVisible();
});

test("消すと 5 秒だけ元に戻せる", async ({ page }) => {
  await addEvent(page, "ジム");
  const item = page.locator(".daypanel").getByRole("button", { name: /ジム/ });
  await item.click();
  ...
  await expect(page.locator(".daypanel").getByRole("button", { name: /ジム/ })).toHaveCount(0);
});

test("日付を押すと選ぶだけで、シートは開かない", async ({ page }) => {
  const grid = page.getByRole("region", { name: "月の表" });
  await grid.locator('[data-date="15"]:not(.out) .hit').click();
  await expect(page.getByRole("dialog")).toHaveCount(0);
  await expect(page).toHaveURL(/date=\d{4}-\d{2}-15/);
  await expect(page.locator(".daypanel .bigday .n")).toHaveText("15");
});

test("長押しすると、その日の予定を足すシートが開く", async ({ page }) => {
  const cell = page.getByRole("region", { name: "月の表" }).locator('[data-date="12"]:not(.out) .hit');
...
test("週と日の表示に切り替えられる", async ({ page }) => {
  ...
  await expect(page.getByRole("region", { name: "週の予定" }).locator(".weekrow")).toHaveCount(7);
  await expect(page.getByRole("button", { name: /会議/ })).toBeVisible();
  await page.getByRole("radio", { name: "日" }).last().click();
  await expect(page.getByRole("region", { name: "日の予定" }).locator(".weekrow")).toHaveCount(1);
});

test("祝日は名前が出る", async ({ page }) => {
  await page.goto("/?date=2026-09-21");
  await expect(page.locator(".daypanel")).toContainText("敬老の日");
  await expect(page.locator(".daypanel .bigday")).toHaveClass(/sun/);
});
