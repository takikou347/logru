 ...
 * @param size 丸の直径。px
 */
export function InitialAvatar({ person, size = 22, className }: { person: AvatarPerson; size?: number; className?: string }) {
 ...
 */
export function AvatarStack({ people, max = 4, size = 18, className }: { people: AvatarPerson[]; max?: number; size?: number; className?: string }) {
    ...
    <span className={cn("inline-flex flex-none items-center", className)} aria-hidden="true">
      {shown.map((p, i) => (
        <InitialAvatar
          ...
          person={p}
          size={size}
          className={cn(ring, i > 0 && "-ml-1.5", p.response === "pending" && "shadow-[inset_0_0_0_1.5px_var(--c),0_0_0_1.5px_var(--glass-flat)]")}
        />
      ))}
      {rest > 0 && (
        <span
          style={{ height: size, minWidth: size, fontSize: Math.round(size * 0.5) }}
          className={cn("-ml-1.5 inline-flex flex-none items-center justify-center rounded-full bg-field-strong px-1 leading-none font-bold text-ink-2", ring)}
        >
          +{rest}
        </span>
      )}
